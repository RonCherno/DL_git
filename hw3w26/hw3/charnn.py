...
def remove_chars(text: str, chars_to_remove):
    ...
    text_clean = "".join([char for char in text if char not in remove_set])
...
def chars_to_labelled_samples(text: str, char_to_idx: dict, seq_len: int, device="cpu"):
    ...
    input_indices = all_indices[:limit]

    samples_flat = torch.zeros(
        (num_samples * seq_len, vocab_size),
        dtype=torch.float32,
        device=device
    )
    samples_flat[
        torch.arange(num_samples * seq_len, device=device),
        input_indices
    ] = 1.0

    samples = samples_flat.view(num_samples, seq_len, vocab_size)

    label_indices = all_indices[1: limit + 1]
...
def hot_softmax(y, dim=0, temperature=1.0):
    ...
    max_val, _ = torch.max(scaled, dim=dim, keepdim=True)
    scaled_stable = scaled - max_val

    exps = torch.exp(scaled_stable)
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        vocab_size = len(char_to_idx)
        init_tens = chars_to_onehot(start_sequence, char_to_idx)
        curr_input = init_tens.unsqueeze(0).to(device=device, dtype=torch.float)
        output, curr_hidden_state = model(curr_input, None)
        last_logits = output[0, -1, :]

        while len(out_text) < n_chars:
            probs = hot_softmax(last_logits, dim=0, temperature=T)
            next_idx = torch.multinomial(probs, 1).item()
            next_char = idx_to_char[next_idx]
            out_text += next_char
            curr_input = torch.zeros((1, 1, vocab_size), device=device, dtype=torch.float)
            curr_input[0, 0, next_idx] = 1.0
            output, curr_hidden_state = model(curr_input, curr_hidden_state)
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        idx = [
            j * num_batches + i
            for i in range(num_batches)  # Iterate over time (batches)
            for j in range(self.batch_size)  # Iterate over streams (batch elements)
        ]
        # ========================
        return iter(idx)
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        # ====== YOUR CODE: ======
        in_dims = [in_dim] + [h_dim]*(n_layers-1)
        for i in range(n_layers):
            layer_comps = nn.ModuleList([
                nn.Linear(in_dims[i], h_dim, bias=True),  # W_x_z
                nn.Linear(h_dim, h_dim, bias=False),  # W_h_z
                nn.Linear(in_dims[i], h_dim, bias=True),  # W_x_r
                nn.Linear(h_dim, h_dim, bias=False),  # W_h_r
                nn.Linear(in_dims[i], h_dim, bias=True),  # W_x_g
                nn.Linear(h_dim, h_dim, bias=False),  # W_h_g
                nn.Dropout(p=dropout)  # dropout_layer
            ])
            self.layer_params.append(layer_comps)

        self.layer_params.append(nn.ModuleList([
            nn.Linear(h_dim, out_dim)
        ]))

        self.layer_params = nn.ModuleList(self.layer_params)
