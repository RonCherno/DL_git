...
def create_train_validation_loaders(
    ...
    valid_size = int(validation_ratio * dataset_size)
    train_size = dataset_size - valid_size

    ds_train = hw1datasets.SubsetDataset(
        source_dataset=dataset,
        subset_len=train_size,
        offset=0
    )

    ds_valid = hw1datasets.SubsetDataset(
        source_dataset=dataset,
        subset_len=valid_size,
        offset=train_size
    )

    sampler_train = torch.utils.data.RandomSampler(ds_train)
    sampler_valid = torch.utils.data.RandomSampler(ds_valid)
