...
class RandomImageDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        ...
        if index < 0 or index >= self.num_samples:
            raise ValueError(f"Index {index} out of range [0, {self.num_samples - 1}]")

        C, H, W = self.image_dim
        shape = (C, H, W)

        # Use the index as the seed for deterministic generation inside the context manager
        with torch_temporary_seed(index):
            # Generate random image: values from 0 to 255 (exclusive 256)
            # using torch.int, consistent with typical image representations.
            image = torch.randint(low=0, high=256, size=shape, dtype=torch.int)

            # Generate random label: integer in [0, num_classes-1]
...
class ImageStreamDataset(IterableDataset):
    ...
    def __iter__(self) -> Iterator[Tuple[Tensor, int]]:
        ...
        # ====== YOUR CODE: ======
        while True:
            image, label = random_labelled_image(
...
class SubsetDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if index < 0 or index >= self.subset_len:
            raise IndexError(
                f"Index {index} is out of bounds for subset of length {self.subset_len}"
            )
        source_index = index + self.offset
        return self.source_dataset[source_index]
        # ========================
