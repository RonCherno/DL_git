...
class CNN(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        for i, out_channels in enumerate(self.channels):
            ...
            # 3. Add POOL every P layers
            if (i + 1) % self.pool_every == 0:
                layers.append(pool_cls(**self.pooling_params))

            # Update current input channels for the next iteration
    ...
    def _n_features(self) -> int:
        ...
        try:
            # ====== YOUR CODE: ======
            # 1. Put the feature extractor in evaluation mode
            self.feature_extractor.eval()

            # 2. Create a dummy tensor with the same shape as input
            with torch.no_grad():
                device = next(self.parameters()).device
                dummy_input = torch.zeros(1, *self.in_size, device=device)
                output = self.feature_extractor(dummy_input)

            self.feature_extractor.train()
            # 3. The output shape will be (1, C_out, H_out, W_out)
    ...
    def _make_mlp(self):
        ...
        full_dims = [*self.hidden_dims, self.out_classes]

        act_cls = ACTIVATIONS[self.activation_type]
        act_params = self.activation_params

        nonlins = []
        for _ in self.hidden_dims:
            nonlins.append(act_cls(**act_params))

        nonlins.append(nn.Identity())
        mlp = MLP(in_dim, full_dims, nonlins)
        # ========================
        return mlp
...
class ResidualBlock(nn.Module):
    ...
    def __init__(
        ...
        for i, (out_channels, kernel_size) in enumerate(zip(channels, kernel_sizes)):
            print(f"[DEBUG] Block Layer {i}: in={curr_in_channels}, out={out_channels}, k={kernel_size}")
            # 1. Add CONV layer
            main_layers.append(
                nn.Conv2d(curr_in_channels, out_channels, kernel_size= kernel_size, padding= (kernel_size//2) ,bias = True)
            )
            #skip for last layer
            if i < len(channels) - 1:
                ...
                # 4. Add Activation
                main_layers.append(act_cls(**activation_params))
    ...
    def forward(self, x: Tensor):
        ...
        out: Tensor = None
        # ====== YOUR CODE: ======
        print(f"[DEBUG 1] Forward Input Shape: {x.shape}")
        #out = self.main_path(x) + self.shortcut_path(x)

        try:
            main_out = self.main_path(x)
            print(f"[DEBUG 2] Main Path Success: {main_out.shape}")
        except Exception as e:
            print(f"[DEBUG 2] Main Path FAILED: {e}")
            raise e

        try:
            short_out = self.shortcut_path(x)
            print(f"[DEBUG 3] Shortcut Success: {short_out.shape}")
        except Exception as e:
            print(f"[DEBUG 3] Shortcut FAILED: {e}")
            raise e

        out = short_out + main_out
        # ========================
        out = torch.relu(out)
        return out
...
class ResidualBottleneckBlock(ResidualBlock):
    ...
    def __init__(
        ...
        full_channels = [inner_channels[-1]] + list(inner_channels) + [in_out_channels]
        full_kernel_sizes = [1] + list(inner_kernel_sizes) + [1]

        print(f"[DEBUG] Bottleneck Config: in_out={in_out_channels}")
        print(f"[DEBUG] Resulting Channels: {full_channels}")
        print(f"[DEBUG] Resulting Kernels: {full_kernel_sizes}")
...
class ResNet(CNN):
    ...
    def _make_feature_extractor(self):
        ...
        for i in range(0, N, P):
            # 1. isolate block channels and kernels
            block_channels = self.channels[i: i + P]
            block_kernels = [3] * len(block_channels)

            if not self.bottleneck:
                # 1. Add a residual block
                layers.append(
                    ResidualBlock(
            ...
            if self.bottleneck:
                ...
                layers.append(
                    ResidualBottleneckBlock(
                        ...
                        inner_channels=block_channels,
                        inner_kernel_sizes=block_kernels,
                        batchnorm=self.batchnorm,
                        dropout=self.dropout,
                        activation_type=self.activation_type,
                        activation_params=self.activation_params
            ...
            # Update current input channels for the next iteration
            curr_in_channels = block_channels[-1]
