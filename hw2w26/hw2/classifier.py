...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        is_positive = positive_probs >= self.threshold
        y_pred = is_positive.int()
        return y_pred
        # ========================
...
def plot_decision_boundary_2d(
    ...
    x1_coords = torch.arange(x1_min, x1_max, dx)
    x2_coords = torch.arange(x2_min, x2_max, dx)

    x1_grid, x2_grid = torch.meshgrid(x1_coords, x2_coords, indexing='ij')


    X_grid = torch.stack((x1_grid.flatten(), x2_grid.flatten()), dim=1)

    with torch.no_grad():
        y_hat_flat = classifier.classify(X_grid)
...
def select_roc_thresh(
    ...
    y_score = y_proba[:, 1].cpu().numpy()
    y_true = y.cpu().numpy()
    fpr, tpr, thresh = roc_curve(y_true, y_score)
    tpr_tensor = torch.from_numpy(tpr)
    fpr_tensor = torch.from_numpy(fpr)
    thresh_tensor = torch.from_numpy(thresh)
    diff = tpr_tensor - fpr_tensor
    optimal_thresh_idx = torch.argmax(diff).item()
    optimal_thresh = thresh_tensor[optimal_thresh_idx].item()
    # ========================
