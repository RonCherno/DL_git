...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        mask = (x > 0)
        d_leaky_relu = torch.full_like(x, self.alpha)
        d_leaky_relu[mask] = 1

        dx = dout * d_leaky_relu
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        log_sum_exp_term = torch.log(sum_exp_x)
        true_class_scores = x[torch.arange(N), y]
        loss_per_sample = log_sum_exp_term - true_class_scores
    ...
    def backward(self, dout=1.0):
        ...
        probabilities = exp_x / sum_exp_x  # Shape (N, D)
        yh = torch.zeros_like(x)
        yh[torch.arange(N), y] = 1.0  # Set the true class indices to 1
        dL_dZ = probabilities - yh
        dx = (1.0 / N) * dL_dZ * dout
        # ========================
...
class Sequential(Layer):
    ...
    def forward(self, x, **kw):
        ...
        for layer in self.layers:
            current_input = layer.forward(current_input, **kw)

        # Finally, assign the result to 'out' so it gets returned by the function.
        out = current_input
        # ========================
    ...
    def backward(self, dout):
        ...
        for layer in reversed(self.layers):
            # The output gradient of the current layer becomes the input
            # gradient for the previous layer.
            current_gradient = layer.backward(current_gradient)

        # The final gradient (w.r.t the initial input x) must be assigned to din
    ...
    def __init__(
        ...
        **kw,
        ...
        for i in range(len(all_features) - 1):
            ...
            in_dim = all_features[i]
            out_dim = all_features[i + 1]

            # 1. Add Fully-Connected (Linear) Layer
            layers.append(Linear(in_dim, out_dim))

            # 2. Check if this is NOT the last layer
            if i < len(all_features) - 2:
                # Add Activation Layer (ReLU or Sigmoid)
                layers.append(ActivationClass())

                # Add Dropout Layer if probability is greater than 0
                if dropout > 0:
