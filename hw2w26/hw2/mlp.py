...
class MLP(nn.Module):
    ...
    def __init__(
        ...
        layers = []
        for i, nonlin in enumerate(nonlins):
            layers.append(nn.Linear(all_dims[i], all_dims[i+1]))
            if isinstance(nonlin, str):
                # Handle string: Lookup, get default kwargs, and instantiate
                activation_cls = ACTIVATIONS[nonlin]
                kwargs = ACTIVATION_DEFAULT_KWARGS[nonlin]
                activation_module = activation_cls(**kwargs)
            else:
                activation_module = nonlin
